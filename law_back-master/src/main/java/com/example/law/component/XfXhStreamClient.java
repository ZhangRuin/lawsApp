...
import com.example.law.pojo.dto.RequestDTO;
...
public class XfXhStreamClient {
    ...
    public String getAuthUrl() {
        try {
            ...
            SecretKeySpec spec = new SecretKeySpec(xfXhConfig.getApiSecretPolling().getBytes(StandardCharsets.UTF_8), "hmacsha256");
            mac.init(spec);
            ...
            // 拼接
            String authorizationOrigin = String.format("api_key=\"%s\", algorithm=\"%s\", headers=\"%s\", signature=\"%s\"", xfXhConfig.getApiKeyPolling(), "hmac-sha256", "host date request-line", sha);
            // 拼接地址
    ...
    public RequestDTO getRequestParam(String uid, List<MsgDTO> msgList) {
        ...
        requestDTO.setHeader(new RequestDTO.HeaderDTO(xfXhConfig.getAppIdPolling(), uid));
        requestDTO.setParameter(new RequestDTO.ParameterDTO(new RequestDTO.ParameterDTO
                .ChatDTO(xfXhConfig.getDomain(), xfXhConfig.getTemperature(), xfXhConfig.getMaxTokens())));

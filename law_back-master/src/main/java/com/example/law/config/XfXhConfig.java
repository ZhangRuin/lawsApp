...
public class XfXhConfig {
    private static int appIdIndex = 0;
    private static int apiKeyIndex = 0;
    private static int apiSecretIndex = 0;
    /**
    ...
    public String getAppIdPolling() {
        String s = appId[appIdIndex];
        appIdIndex = (appIdIndex + 1) % appId.length;
        return s;
    }

    public String getApiKeyPolling() {
        String s = apiKey[apiKeyIndex];
        apiKeyIndex = (apiKeyIndex + 1) % apiKey.length;
        return s;
    }

    public String getApiSecretPolling() {
        String s = apiSecret[apiSecretIndex];
        apiSecretIndex = (apiSecretIndex + 1) % apiSecret.length;
        return s;
    }
}

package com.example.law.listener;

import com.alibaba.fastjson.JSONObject;
import com.example.law.pojo.dto.MsgDTO;
import com.example.law.pojo.dto.ResponseDTO;
...
public class XfXhWebSocketListener extends WebSocketListener {
    ...
    public void onMessage(@NotNull WebSocket webSocket, @NotNull String text) {
        ...
        if (responseData.getHeader().getCode() != 0) {
            String answer = "发生错误，错误码为：" + responseData.getHeader().getCode() + "; " +
                    "信息：" + responseData.getHeader().getMessage();
            // 日志记录
            log.error(answer);
            // 设置回答

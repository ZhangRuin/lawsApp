  ...
  data: {
    ...
    content: '',
        ...
        chatList: [{
                ...
                nickname: '劳动争议解决AI',
                avatar: '/pages/images/GPT.png',
                message: '您好，很高兴为您提供劳动争议解决服务。请问您在工作中遇到过什么争议问题需要解决吗？如果对回答不满意，点击屏幕左下角即可切换人工服务哦~(提供的法律建议仅供参考，不具备任何法律效力)',
                type: 'text',
            }
        ],
    },
    ...
    data: {
      ...
      this.setData({
          ...
          let timer2 = setTimeout(() => {
            clearTimeout(timer2);
            let chatList = list.concat(msg).concat(this.data.loadingMsg); // 初始化 chatList
...
scrollToBottom() {
    ...
    // wx.request({
    //   url: 'http://49.235.175.168:9000/api/law/dispute',
    //   data:{
    ...
    //       return
    //     }
    //     console.log("api/dispute:")
    //     console.log(res.data)
    //     let aiAnswers = [];
    //     var num = res.data.result.disputeQuestions.length
    //     for(let i = 0; i<num+1 ;i++){
    //       if(i == 0){
    //         aiAnswers.push({
    //           msgId: '2022',
    ...
    //         nickname: '劳动法律咨询AI',
    //         avatar: '/pages/images/GPT.png',
    //         message: i+"、"+res.data.result.disputeQuestions[i - 1].aiTitle + ':\n' + "\t\t\t\t" + res.data.result.disputeQuestions[i-1].answer,
    //         type: 'text'
    //       });
    //     }
    //     this.setData({

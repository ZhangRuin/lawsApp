  ...
  data: {
    ...
    content: '',
        ...
        chatList: [{
                ...
                nickname: '法律文书服务AI',
                avatar: '/pages/images/GPT.png',
                message: '您好，很高兴为您提供法律文书服务。请问您需要什么类型的法律文书模板？如果对回答的模板不满意，可点击屏幕左下角切换人工服务哦~',
                type: 'text',
            }
        ],
    },
...
sendClick() {
  ...
  wx.request({
    url: 'https://lawaaaa.online/api/template',
    data:{
      ask: this.data.content
    },
    method: 'POST',
    header:{
    ...
    success: (res)=>{
      ...
      for(let i = 1; i<num+1 ;i++){
        aiAnswers.push({
          ...
          nickname: '法律文书服务AI',
          avatar: '/pages/images/GPT.png',
          message: i+"."+res.data.result[i-1].docCategory  + "模板链接:"+ '\n'+res.data.result[i-1].docLink ,
          type: 'text'
        });
      }
      this.setData({
  ...
  this.setData({
    ...
    let timer2 = setTimeout(() => {
      ...
      for (let i = 0; i < this.data.aiAnswer.length; i++) {
        chatList.push(this.data.aiAnswer[i]); 
      }
